...
(async () => {
  ...
  const result = await page.evaluate(async () => {
    ...
    const analyticsResult = {
      ...
      refinedChartPalette: charts.daily?.config?.type === "line" && charts.daily?.config?.data?.datasets?.[0]?.borderColor === "#234767" && charts.daily?.config?.data?.datasets?.[1]?.borderColor === "#b49a6a" && charts.analyticsPayment?.config?.data?.datasets?.[0]?.backgroundColor?.join(",") === "#234767,#65879a,#d8dde3",
      fixedBarThickness: charts.employee?.config?.data?.datasets?.[0]?.barThickness === 12 && charts.customer?.config?.data?.datasets?.[0]?.barThickness === 12,
