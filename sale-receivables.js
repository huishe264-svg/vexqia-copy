...
openSaleEditor=function(id){
  ...
  if($("collectReceivableBtn"))$("collectReceivableBtn").onclick=async()=>{const date=$("recoveryBusinessDate").value,amount=Number($("recoveryAmount").value),method=$("recoveryMethod").value;if(!date||amount<=0||!method)return showStatus("回収日・金額・支払方法を入力してください","error");const button=$("collectReceivableBtn");button.disabled=true;button.textContent="回収登録中…";try{const collected=await db.rpc("collect_receivable_payment",{target_store_id:storeId,target_receivable_sale_id:s.id,target_business_date:date,target_received_amount:amount,target_payment_method:method});if(collected.error)throw collected.error;closeModal("saleEditModal");await loadAll();if(currentSalesDate===date)renderSalesDay(date);showStatus(`${date}の未精算売上として${yen(amount)}を回収登録しました。日別精算で確定してください。`,"success")}catch(error){button.disabled=false;button.textContent="この日の売上として回収登録";showStatus(friendlyErrorMessage(error,"未収の回収登録に失敗しました。画面を更新してもう一度お試しください。"),"error")}}
};
...
const renderSalesDayBeforeBankTransfer=renderSalesDay;
renderSalesDay=function(date){
  renderSalesDayBeforeBankTransfer(date);
  const amount=totalSales(recoveredSales(sales.filter(s=>s.business_date===date&&s.payment_method==="銀行振込")));
  if(amount){$("salesDayDetail").querySelector(".daily-payment-grid")?.insertAdjacentHTML("beforeend",`<div class="daily-payment-card"><div class="key">銀行振込</div><div class="value">${yen(amount)}</div><div class="sub">回収済み・レジ金への加算なし</div></div>`)}
};
