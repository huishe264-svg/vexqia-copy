...
(async()=>{
 ...
 try {
  ...
  const result=await page.evaluate(async()=>{
   currentAuthUser={id:'owner'};currentStoreMember={user_id:'owner',role:'owner'};storeId='test-store';storeMembers=[currentStoreMember];
   ...
   openSaleEditor('old');$('recoveryBusinessDate').value='2026-09-14';$('recoveryMethod').value='銀行振込';await $('collectReceivableBtn').onclick();
   ...
   return {request,dayText,editMethod:$('editSaleMethod').value,visits:directVisitCount('c'),total:totalSales(recoveredSales(sales)),cash:totalSales(recoveredSales(sales).filter(s=>s.payment_method==='現金'))};
  });
  ...
  assert.equal(result.editMethod,'銀行振込');assert.equal(result.visits,1);assert.equal(result.total,30000);assert.equal(result.cash,0);assert.match(result.dayText,/銀行振込/);assert.match(result.dayText,/レジ金への加算なし/);
