...
openSaleEditor=function(id){openSaleEditorBeforeDemoSettings(id);if(!demoSettingsEnabled())return;const configured=operatingValue("payment_methods"),editSelect=$("editSaleMethod"),recoverySelect=$("recoveryMethod");if(editSelect){const current=editSelect.value,methods=[...new Set([...configured,...(current?[current]:[])])];editSelect.innerHTML='<option value="">選択してください</option>'+methods.map(method=>`<option ${method===current?"selected":""}>${esc(method)}</option>`).join("")}if(recoverySelect){const current=recoverySelect.value,methods=[...new Set([...configured,"銀行振込"])];recoverySelect.innerHTML='<option value="">選択してください</option>'+methods.map(method=>`<option ${method===current?"selected":""}>${esc(method)}</option>`).join("")}replaceTextIn($("saleEditBody"),"出前・タバコ等",operatingValue("deduction_label"))};
