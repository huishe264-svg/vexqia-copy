-- A collection-only record is revenue on the selected date, not a new visit.
...
alter table public.sales add constraint sales_party_size_check
  check ((record_type = 'receivable_payment' and party_size = 0)
    or (record_type <> 'receivable_payment' and party_size > 0));
...
begin
  if caller is null or not public.is_store_member(target_store_id) then raise exception 'この店舗の未収を回収する権限がありません' using errcode='42501'; end if;
  if target_business_date is null or target_received_amount <= 0 or target_payment_method not in ('現金','カード') then raise exception '回収日・金額・支払方法を確認してください' using errcode='22023'; end if;
  select * into old_sale from public.sales where id=target_receivable_sale_id and store_id=target_store_id for update;
  if not found or old_sale.payment_status<>'未収' or old_sale.recognized_via_sale_id is not null then raise exception 'この未収はすでに回収済みか、現在利用できません' using errcode='22023'; end if;
  if not public.is_store_owner_or_manager(target_store_id) and old_sale.employee_id is distinct from public.current_store_employee_id(target_store_id) then raise exception 'この未収を回収できるのは、紐づいた口座または店舗管理者です' using errcode='42501'; end if;
  ...
  insert into public.sale_receivable_links(store_id,payment_sale_id,receivable_sale_id,original_receivable_amount,created_by) values(target_store_id,payment_sale.id,old_sale.id,old_sale.total_amount,caller);
