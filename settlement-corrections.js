...
.pull-refresh-indicator{position:fixed;top:14px;left:50%;z-index:115;display:flex;align-items:center;justify-content:center;gap:9px;min-width:130px;padding:8px 13px;border:1px solid #d9dee6;border-radius:999px;background:#fff;box-shadow:0 8px 24px rgba(19,34,56,.14);color:var(--navy);font-size:12px;font-weight:850;text-align:center;opacity:0;transform:translate(-50%,-62px);transition:transform .18s ease,opacity .18s ease}.pull-refresh-indicator.show{opacity:1;transform:translate(-50%,0)}.pull-refresh-spinner{display:none;width:18px;height:18px;border:2px solid #d9dee6;border-top-color:var(--navy);border-radius:50%}.pull-refresh-indicator.loading .pull-refresh-spinner{display:block;animation:pullRefreshSpin .7s linear infinite}@keyframes pullRefreshSpin{to{transform:rotate(360deg)}}.app>header,.app>main,.app>#rolePreviewBanner{transition:transform .2s ease}.pull-refreshing .app>header,.pull-refreshing .app>main,.pull-refreshing .app>#rolePreviewBanner{transform:translateY(58px)}.settlement-correction-card{border-color:#dac79d}.settlement-correction-warning{margin-bottom:10px;padding:11px;border-radius:10px;background:#fff8e8;color:#665124;font-size:12px;line-height:1.6}
</style>`);
...
(function setupPullToRefresh(){
  const indicator=document.createElement("div");indicator.id="pullRefreshIndicator";indicator.className="pull-refresh-indicator";indicator.innerHTML='<span class="pull-refresh-spinner" aria-hidden="true"></span><span class="pull-refresh-label">↓ 引っ張って更新</span>';document.body.appendChild(indicator);const label=indicator.querySelector(".pull-refresh-label");
  ...
  document.addEventListener("touchmove",event=>{if(!tracking)return;distance=Math.max(0,event.touches[0].clientY-startY);if(distance>18){indicator.classList.add("show");label.textContent=distance>=75?"↑ 離して更新":"↓ 引っ張って更新"}},{passive:true});
  document.addEventListener("touchend",async()=>{if(!tracking)return;tracking=false;const shouldRefresh=distance>=75;distance=0;if(!shouldRefresh){indicator.classList.remove("show");return}refreshing=true;document.body.classList.add("pull-refreshing");indicator.classList.add("show","loading");label.textContent="更新中…";try{await loadAll();renderPage(currentPage);indicator.classList.remove("loading");label.textContent="✓ 更新しました";setTimeout(()=>{indicator.classList.remove("show");document.body.classList.remove("pull-refreshing")},650)}catch(error){indicator.classList.remove("loading");label.textContent="更新できませんでした";showStatus(friendlyErrorMessage(error,"データを更新できませんでした。"),"error");setTimeout(()=>{indicator.classList.remove("show");document.body.classList.remove("pull-refreshing")},1100)}finally{refreshing=false}}
  ,{passive:true});
})();
