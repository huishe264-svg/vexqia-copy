...
assert.match(html,/data-owner-only data-page="finance"/);assert.match(sql,/storage\.buckets/);assert.match(sql,/public\.is_store_owner\(store_id\)/);assert.match(sql,/get_owner_monthly_finance/);assert.match(sql,/sale_payment_events/);assert.match(js,/現金払いだけレジ金から差し引きます/);assert.match(js,/経費明細CSVを出力/);
...
const result=await page.evaluate(async()=>{currentAuthUser={id:"owner"};currentStoreMember={user_id:"owner",role:"owner"};storeId="store";platformAdminMode=false;storeOperatingSettings={expense_management_mode:"full"};
